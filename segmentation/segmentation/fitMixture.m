function r = QuestionB

%************* Code for QUESTION B *****************
% load the first mixture to fit the data 
load('RGBTrained.mat','RGBAppleFinal','RGBNonAppleFinal');

% I decided to use 7 gaussians at this time then, I will try to obtain a
% better result by varying the number of Gaussians I use
...
save('RGBMixtureTraining7.mat', 'mixGaussEstApple', 'mixGaussEstNonApple') ;

%************* Code adding for QUESTION D *****************
nGaussEst = 3 ;
%fit mixture of Gaussian model for apple data
mixGaussEstApple = fitMixGauss(RGBAppleFinal,nGaussEst);
%fit Mixture of Gaussian model for non-apple data
mixGaussEstNonApple = fitMixGauss(RGBNonAppleFinal,nGaussEst);
save('RGBMixtureTraining3.mat', 'mixGaussEstApple', 'mixGaussEstNonApple') ;
% 
nGaussEst = 5 ;
%fit mixture of Gaussian model for apple data
mixGaussEstApple = fitMixGauss(RGBAppleFinal,nGaussEst);
%fit Mixture of Gaussian model for non-apple data
mixGaussEstNonApple = fitMixGauss(RGBNonAppleFinal,nGaussEst);
save('RGBMixtureTraining5.mat', 'mixGaussEstApple', 'mixGaussEstNonApple') ;
...
%fit Mixture of Gaussian model for non-apple data
mixGaussEstNonApple = fitMixGauss(RGBNonAppleFinal,nGaussEst);
save('RGBMixtureTraining10.mat', 'mixGaussEstApple', 'mixGaussEstNonApple') ;
...
for cGauss =1:k
    mixGaussEst.cov(:,:,cGauss) = (1.0+1.5*rand(1))*eye(nDim,nDim);
end;
...
for cIter = 1:nIter
    ...
    for cGauss = 1:k
        ...
        postHiddenArray = repmat(postHidden(cGauss,:),3,1) ;
        ...
        mixGaussEst.cov(:,:,cGauss) = ((repmat(postHidden(cGauss,:), 3,1).*data2)*data2.')./sum(postHidden(cGauss,:)) ;
